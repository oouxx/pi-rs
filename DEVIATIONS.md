...
| `bun/mod.rs` — session 快照推送（ctx.sessionManager / 同步读 API） | 扩展与宿主同进程：`ctx.sessionManager` 就是真实 `SessionManager` 实例，`pi.getActiveTools()`/`getCommands()`/`getThinkingLevel()`/`getSessionName()` 等同步读 API 直接读共享状态，**永远新鲜、零成本** | 宿主把同步读 API 的完整快照（session: cwd/sessionDir/sessionId/sessionFile/sessionName/leafId/header/entries/labels + activeTools/allTools/commands/thinkingLevel/modelId）随 `fire_event`/`execute_tool`/`execute_command` 推给 Bun 侧（`message_update` 流式热路径除外，每 token 一次避免序列化整条 branch），bootstrap 缓存后供同步 API 读取；`getBranch(fromId)`/`getTree()`/`buildContextEntries()`/`getLeafEntry()`/`getEntry(id)`/`getLabel()`/`getEntries()`/`getHeader()` 从 entries 派生（对齐 TS SessionManager 语义） | 子进程架构的必然代价：`getBranch()` 等在扩展里是**同步调用**（直接 `branch.length`），无法用异步 RPC 往返，只能推缓存。快照在事件/工具触发时刻与宿主状态一致；`message_update` 内读 branch 会拿到上一次推送的快照（正常扩展不会在每 token 的 handler 里读整条 branch） | 已确认保留 |
