...
| ---------------------- | ---------- | -------------- | -------- | -------- |
| 自解释文档（system prompt + TUI 启动 header） | TS 在 system prompt 注入 "Pi documentation" 段（`readmePath`/`docsPath`/`examplesPath` 绝对路径 + 各文档索引，`system-prompt.ts:139-144`），并在启动 header 紧凑/展开两态末尾显示 onboarding 行 `Pi can explain its own features and look up its docs. Ask it how to use or extend Pi.`（`interactive-mode.ts:952`） | **2026-09-22 用户决定删除**：`build_system_prompt` 不再注入文档路径段，`system_prompt.rs` 的 `get_readme_path`/`get_docs_path`/`get_examples_path` 局部 helper 及 `use crate::config` 移除；`pi-tui/src/app.rs::header_lines` 不再渲染 onboarding 行（紧凑态 3 行：logo/快捷键/press 提示；展开态 19 条快捷键，尾部空行一并移除） | pi-rs 未随包发布 `docs/`（仓库仅 1 个 `ui-protocol.md`）、无 `examples/`，prompt 解析出的 `~/.pi-rs/{README.md,docs,examples}` 均不存在，功能实际不可用；用户选择删除功能而非补齐并发布文档资源 | 已确认保留 |
...
| TUI 渲染层（对应原 `packages/coding-agent` 内 TUI 组件，即 `pi-tui`） | 原版 TUI 组件的具体渲染实现 | **2026-08-23 起已纳入范围（最小可用版）**：恢复历史 Elm 架构 pi-tui（ratatui 0.29），markdown 渲染整体改用 vendored grok-build `xai-grok-markdown`（见 THIRD-PARTY-NOTICES.md），不按原版自研。**2026-08-23 第三轮（样式对齐）**：主视图布局与配色已按 TS 原版 interactive 模式复刻——dark.json 调色板（accent `#8abeb7`、text `#d4d4d4`、userMessageBg `#343541`、toolPending/Success/ErrorBg `#282832/#283228/#3c2828` 等）、用户消息全宽背景盒、工具调用状态色盒（粗体标题+灰色输出）、移除顶栏，改为 TS 式底部 dock（spinner 状态行 + `─` 边框编辑器 + 两行 footer：dim cwd(branch) + 着色 context% + 右对齐模型名）。**2026-08-23 第六轮（像素级复刻，复用 vendor）**：① 启动 header（TS `builtInHeader` ExpandableText）——logo `Pi v1.83.1`（accent 粗体 + dim）+ 紧凑/展开两态快捷键提示（`escape interrupt · ctrl+c/ctrl+d clear/exit · / commands · ! bash · ctrl+o more`，展开列出全部 19 条），Ctrl+O（`app.tools.expand`）切换并同时展开全部工具输出；② footer 两行完整对齐 TS `FooterComponent`——line 1 `pwd (branch) • sessionName`，line 2 `↑in ↓out Rcache Wcache CH% $cost ctx%/window (auto)`（context 阈值着色 error>90/warning>70，`?/window` 未知态）+ 右对齐 `(provider) model • thinking`（reasoning 模型显示 thinking level）；③ 工具调用渲染 TS fallback 格式（粗体标题 + 空行 + `JSON.stringify(args, null, 2)` 默认色 + 输出）；④ assistant thinking 块（`thinkingText` 斜体）+ 终止原因提示（length/aborted/error 三态，error 色，对齐 TS `AssistantMessageComponent`）；⑤ 转录内容适配 TS ScrollView 锚定语义（内容不足一屏时顶对齐，溢出时跟随底部）；⑥ 状态栏/编辑器/对话框保持前几轮形态。工具输出预览 10 行 + `... (N more lines, ctrl+o to expand)` 提示（TS fallback 语义），Ctrl+O 展开。**2026-08-24 第七轮（工具块像素对齐）**：bash 工具走 TS 专用 renderer（`core/tools/bash.ts`）——`$ {command}` 粗体标题 + muted ` (timeout Ns)` 后缀、输出未展开时预览**尾部 5 视觉行** + 前导 `... (N earlier lines, ctrl+o to expand)`（`BASH_PREVIEW_LINES`=5）、muted `Elapsed {x.x}s`（运行中，tick 驱动每秒刷新）/ `Took {x.x}s`（完成，`formatDuration` 同款 `(ms/1000).toFixed(1)`）；其他工具保持 fallback（10 行尾部提示、无计时器）；**输出统一 toolOutput 色**——移除此前 Rust 增强的 diff +/- 着色（TS renderer 无此行为）。**2026-08-24 第八轮（主屏幕模式 + 工具块位置 + renderer 差距）**：①**主屏幕差分渲染**（对齐 TS `TuiMainScreen`）——移除 alternate screen，整个 UI 渲染进终端主屏幕（差分输出 + resize 清屏重放）；**2026-08-24 用户决定回退①**：主屏幕模式因退出清理缺失与 resize 尺寸错误问题回退为 alt screen + ratatui 直渲（`EnterAlternateScreen`/`LeaveAlternateScreen` + `ratatui_terminal().draw()`，恢复 HEAD 基线；详见 PORTING_MISTAKES.md #118），②③④保留；②**工具块位置**：tools 与 messages 按共享 block-id 时间序穿插（TS chatContainer 顺序——工具块紧跟请求它的 assistant 消息之后，不再全部堆在消息上方）；③**read/grep/edit 紧凑 renderCall**：read `read {path}{:range}`（accent path + warning 范围，未展开只显示标题——TS `formatReadResult` 空串）、grep `grep /{pattern}/ in {path} ({glob}) limit {n}` + 15 行预览、edit `edit {path}` + diff 文本（TS 的 diff widget 渲染未复刻，见差距）；④**bash/read 截断警告**：`[Full output: path. Truncated: showing N of M lines]` warning 色（从结果 `details.truncation` 提取）。**2026-09-05 第十轮（消息排队对齐，TS interactive-mode handleSubmit/handleFollowUp/handleDequeue/queueCompactionMessage/flushCompactionQueue）**：①**Alt+Enter 排 follow-up**（TS `app.message.followUp`）——streaming 时按键时刻入队（镜像推入 + `Agent.steer/follow_up` 绕过 session 锁：命令任务整个 run 期间持锁，经通道会排到 run 结束后使 is_streaming 判定与 pending 显示失效；按键到 effect 执行之间 run 已结束时由 `RunSettledContinuations` 触发消费），压缩中入 TUI 层 compaction 队列（TS `queueCompactionMessage`），空闲等同 Enter（完整命令路由，TS `editor.onSubmit`）；Shift+Enter 仍为换行（TS `tui.input.newLine`）；②**Alt+Up 取回排队消息**（TS `app.message.dequeue` → `restoreQueuedMessagesToEditor`）——按 `getAllQueuedMessages` 顺序（session steering + compaction steer，然后 session follow-up + compaction follow-up）合并进编辑器，`\n\n` 连接，状态行 `Restored N queued message(s) to editor` / `No queued messages to restore`；③**pending 显示区**（TS `pendingMessagesContainer`）——dock 顶部新增排队区：dim `Steering: …`/`Follow-up: …` 行（`TruncatedText` 单行截断）+ `↳ alt+up to edit all queued messages` 提示，会话层由 `QueueUpdate` 事件（TS `queue_update` → `updatePendingMessagesDisplay`）驱动，入队时 UI 本地即时更新；④**压缩期间排队 + flush**（TS `compactionQueuedMessages`/`flushCompactionQueue`，对齐差距 #26）——Enter/Alt+Enter 压缩中入 compaction 队列（扩展命令仍立即执行），`compaction_end` 后 flush（扩展命令先立即执行、第一条非命令消息作新 prompt、其余按模式入 steer/followUp；失败清队列还原并提示，TS `restoreQueue`；aborted 时也 flush）；Esc 在压缩中 = `abortCompaction`（无锁 `CompactionAbortHandle`，压缩运行在持锁任务里经通道中止会排在锁后）；status 区显示 TS `CompactionStatusIndicator` 同款标签（manual/overflow/auto + esc to cancel）；⑤**排队消息不进气泡**（TS `message_start` role==user 语义）——提交路径不再直接画用户气泡，bridge 转发 `MessageStart(User)` → 聊天区（排队中的消息只在 pending 区显示，被消费后才进转录；Esc 中断还原后不再残留未发送的气泡）；⑥**run 级 spinner**——新增 `AgentStart/AgentEnd` 事件映射（TS `isStreaming = _isAgentRunActive`）：同一 run 的多条消息与工具执行间保持 spinner，排队消息消费触发的续跑也能驱动 spinner（此前 MessageEnd 每条复位）；`prompt()` 补 TS 同款手动压缩中抛错（"Cannot submit a prompt while compaction is in progress"，TUI 在自己层排队不受影响，RPC/编程调用方报错）。**2026-09-05 第十一轮（/compact + provider 重试显示）**：①**`/compact [instructions]`**（TS `handleCompactCommand` → `session.compact()`）——先无锁 abort 在飞 run（TS compact() 同款 `_disconnectFromAgent` + `abort()`）再由任务执行；错误/结果经 `CompactionEnd` 事件回显（TS 忽略返回值），补全列表 + `/help` 同步；manual 压缩补齐 TS 接线：`compact()` 注册 `compaction_abort`（`is_compacting()`/`abort_compaction()`/`prompt()` 压缩中抛错此前对 manual 全失效）+ 完结时发 `CompactionEnd`（先清 abort 再 emit，对齐 TS #3852cb2b8 顺序，aborted → "Compaction cancelled"）；②**重试显示**（TS `RetryStatusIndicator` + `CountdownTimer`）——`auto_retry_start`/`summarization_retry_scheduled` → status 区 warning 色 spinner + 逐秒递减 `Retrying (n/m) in Xs... (esc to cancel)`（Tick 驱动，`started_at` 实时计算剩余秒）；`auto_retry_end` → 清除，仅最终失败时报 `Retry failed after N attempts: {finalError}`；`summarization_retry_scheduled` 先 showError 再倒计时；`summarization_retry_attempt_start/finished` 清倒计时回落压缩标签（branchSummary 源 Rust 未实现分支摘要，不适用）；Esc 在自动重试退避中 = `abortRetry`（无锁 `RetryAbortHandle`，退避在持锁任务里；TS auto_retry_start 替换 onEscape），压缩重试倒计时期间 Esc 仍走 abortCompaction（TS 不替换 onEscape）。新增 E2E：重试倒计时 + Esc 中止（"Retry cancelled"）+ 后续消息正常跑；/compact 事件回显（小会话 "Nothing to compact (session too small)"）。 | 用户决定：先恢复最小可用，再逐模块对齐原版 | 已确认保留 |

| App-owned 文本选择：组件级鼠标分发（`pi-tui` app.rs `handle_mouse`） | TS `TuiAltScreen.handleMouseEvent` 先把鼠标事件分发给布局组件（`MouseRegion`、textarea、OSC 8 超链接激活、右键粘贴）；只有无人处理时才走全屏选择 | 仅实现全屏选择本身：press/drag/release → 选择；不做组件级鼠标分发、不激活 OSC 8 超链接、无右键粘贴、不点选编辑器光标 | 原版这些能力依赖组件级 `handleMouse` 基础设施，pi-rs 组件层尚未移植；全屏选择是用户本轮明确要求的核心 | 已确认保留 |
| App-owned 文本选择：复制反馈（`pi-coding-agent` interactive.rs `AgentCmd::CopySelection`） | TS `flash("Copied!")` 在 alt screen 顶部叠加瞬时提示；失败时 `flash(error, 5000)` | 成功后写入 system 消息 `Copied!`，失败写 `Failed to copy to clipboard: {e}`（与 Ctrl+X/`/copy` 同款，沿用本 port 无 flash 层的既定做法） | pi-tui 无 flash 叠加层；system 消息是既有的复制反馈通道 | 已确认保留 |
| App-owned 文本选择：CJK 分词（`pi-tui` selection.rs `word_range`） | `Intl.Segmenter`（word 粒度）对 CJK 用词典分词，双击 `中文` 选中整个词 | Rust 用 UAX #29 `split_word_bounds`（无 CJK 词典），双击选中单个表意字 | 复刻 ICU 词典分词超出范围；字符级选择与 grapheme 吸附仍正确 | 已确认保留 |
| App-owned 文本选择：自动滚动节奏（`pi-tui` app.rs `selection_auto_scroll`） | `setInterval(..., 50)` 每 50ms 滚动一行 | 由 interactive 主循环 tick 驱动（100ms/行） | pi-tui 无独立定时器；tick 驱动与现有 spinner/重试倒计时一致 | 已确认保留 |
| App-owned 文本选择：内容快照上限（`pi-tui` app.rs `MAX_SELECTION_CONTENT_ROWS`） | 每帧物化整份文档（`scrollContentLines`），无上限 | 仅在按下时物化一次，>20_000 行时内容空间选择不可用（屏幕空间选择仍可用） | 避免超长会话 OOM；正常长度会话无差异 | 已确认保留 |
...
| 渲染策略：行级差分渲染（interactive.rs 主循环 / app.rs `run()`，`pi-tui/src/line_screen.rs`） | TS `TuiAltScreen.doRender` / `TuiMainScreen.doRender`（grok-build）：整行比较、整行重写（`ESC[{row};1H` + `ESC[2K` + 整行），首帧/尺寸变化时全量清屏重绘（`ESC[2J`），更新批次用 synchronized output（`ESC[?2026h/l`）包裹 | **2026-08-25 起同款行级差分**：`view()` 渲染进 ratatui buffer → `buffer_to_lines` 序列化为 ANSI 行 → 与上一帧逐行比较，只重写变化的行；首帧/尺寸变化全量重绘；synchronized output 包裹。**2026-08-25 曾短暂用每帧 `Terminal::clear()` + 全量重绘（已废弃）** | 行级整行重写不存在 cell 级局部更新，宽字符（CJK）不会被半个覆盖——终端上不会留下"每帧固定在原位"的陈旧字符（ratatui cell 级 diff 在 tmux / Windows Terminal 等终端上有此残留问题）。与 TS 原版（grok-build）策略一致；PTY e2e 用 ANSI 模拟器验证滚动不变量（↑↓ 往返、gg/G、流式、resize） | 已确认保留 |
