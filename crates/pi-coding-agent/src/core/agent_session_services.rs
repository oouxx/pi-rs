...
pub async fn create_agent_session_services(
    ...
    // Wire the auth.json resolver so stored credentials participate in auth
    // checks and requests (match TS `getAuth`); reads the file fresh per lookup.
    model_registry.wire_auth_resolver(Path::new(&agent_dir).join("auth.json"));
