...
pub fn create_grep_tool(cwd: &str, _options: Option<GrepToolOptions>) -> AgentTool<serde_json::Value, serde_json::Value> {
    ...
    AgentTool {
        ...
        execute: Arc::new(move |_tool_call_id: String, params: serde_json::Value, _signal: Option<tokio::sync::watch::Receiver<bool>>| {
            ...
            Box::pin(async move {
                ...
                let context_lines = params.get("context").and_then(|v| v.as_u64()).map(|v| v as usize).unwrap_or(0);
                ...
                let regex_pattern = if literal { regex::escape(pattern) } else { pattern.to_string() };
                let re = match regex::RegexBuilder::new(&regex_pattern).case_insensitive(ignore_case).build() {
                    Ok(re) => re,
                    Err(e) => {
                        return Ok(AgentToolResult {
                            content: vec![ContentBlock::text(format!("Invalid regex pattern: {}", e))],
                ...
                if absolute_path.is_file() {
                    if let Err(e) = grep_file(&absolute_path_str, &re, context_lines, &mut output_lines, &mut match_count, limit, &mut match_limit_reached) {
                        return Ok(AgentToolResult {
                            content: vec![ContentBlock::text(format!("Error searching file: {}", e))],
                            details: serde_json::Value::Null,
                            terminate: None,
                        });
                    }
                    ...
                    if let Err(e) = search_directory(&absolute_path, &re, &glob_matcher, context_lines, &mut output_lines, &mut match_count, limit, &mut match_limit_reached) {
                        return Ok(AgentToolResult {
                            content: vec![ContentBlock::text(format!("Error searching directory: {}", e))],
                            details: serde_json::Value::Null,
                            terminate: None,
                        });
                    }
                    ...
                    return Ok(AgentToolResult {
                        content: vec![ContentBlock::text(format!("Path not found: {}", search_path))],
                        details: serde_json::Value::Null,
                        terminate: None,
                    });
                }
                ...
                if truncation.truncated { details.truncation = Some(truncation.clone()); }
                if let Some(reached) = match_limit_reached { details.match_limit_reached = Some(reached); }

                Ok(AgentToolResult {
                    content: vec![ContentBlock::text(if truncation.truncated { truncation.content } else { output })],
...
fn grep_file(
    file_path: &str, re: &regex::Regex, context_lines: usize,
    output_lines: &mut Vec<String>, match_count: &mut usize, limit: usize,
    ...
    for (i, line) in lines.iter().enumerate() {
        if *match_count >= limit { *match_limit_reached = Some(limit); return Ok(()); }
        if re.is_match(line) {
            *match_count += 1;
            if context_lines > 0 {
...
fn search_directory(
    dir: &std::path::Path, re: &regex::Regex, glob_matcher: &Option<glob::Pattern>,
    context_lines: usize, output_lines: &mut Vec<String>, match_count: &mut usize,
    limit: usize, match_limit_reached: &mut Option<usize>,
) -> Result<(), String> {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            search_directory(&path, re, glob_matcher, context_lines, output_lines, match_count, limit, match_limit_reached)?;
            if match_limit_reached.is_some() { return Ok(()); }
        } else if path.is_file() {
            if let Some(ref matcher) = glob_matcher {
                let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
                if !matcher.matches(file_name) { continue; }
            }
            let path_str = path.to_string_lossy().to_string();
            grep_file(&path_str, re, context_lines, output_lines, match_count, limit, match_limit_reached)?;
            if match_limit_reached.is_some() { return Ok(()); }
        }
    }
    Ok(())
