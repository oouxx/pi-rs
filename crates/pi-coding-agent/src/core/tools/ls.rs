...
pub fn create_ls_tool(cwd: &str, _options: Option<LsToolOptions>) -> AgentTool<serde_json::Value, serde_json::Value> {
            ...
            Box::pin(async move {
                ...
                if !absolute_path.is_dir() {
                    return Ok(AgentToolResult {
                        content: vec![ContentBlock::text(format!("Not a directory: {}", dir_path))],
                ...
                let entries = match tokio::fs::read_dir(&absolute_path_str).await {
                    ...
                    Err(e) => {
                        return Ok(AgentToolResult {
                            content: vec![ContentBlock::text(format!("Error reading directory: {}", e))],
                            details: serde_json::Value::Null,
                            terminate: None,
                        });
                    }
                ...
                if entries.len() > limit { details.entry_limit_reached = Some(limit); }
