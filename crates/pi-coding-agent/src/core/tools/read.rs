...
pub fn create_read_tool(cwd: &str, options: Option<ReadToolOptions>) -> AgentTool<serde_json::Value, serde_json::Value> {
        ...
        execute: Arc::new(move |_tool_call_id: String, params: serde_json::Value, _signal: Option<tokio::sync::watch::Receiver<bool>>| {
            ...
            Box::pin(async move {
                ...
                let bytes = match operations.read_file(&absolute_path_str).await {
                    ...
                    Err(e) => {
                        return Ok(AgentToolResult {
                            ...
                            details: serde_json::to_value(ReadToolDetails::default()).unwrap_or(serde_json::Value::Null),
                            terminate: None,
                        });
                    }
                };
                ...
                let text_content = match String::from_utf8(bytes) {
                    Ok(s) => s,
                    Err(_) => {
                        let file_size = absolute_path.metadata().map(|m| m.len() as usize).unwrap_or(0);
                        return Ok(AgentToolResult {
                ...
                let user_limited_lines = limit.map(|lim| std::cmp::min(lim, all_lines.len() - start_line));
                ...
                if truncation.first_line_exceeds_limit {
                    ...
                    output_text = if truncated_by == "lines" {
                        format!(
                            ...
                            truncation.content, start_line + 1, end_line_display, total_file_lines, next_offset
                        )
                    } else {
                        format!(
