...
/// Sends a prompt to the agent and outputs the result.
pub async fn run_print_mode(options: PrintModeOptions<'_>) -> i32 {
    match options.mode {
        "json" => run_json_mode(options.session, options.message).await,
        _ => run_text_mode(options.session, options.message, options.verbose).await,
    }
}

/// Run in text mode: stream response to stdout.
async fn run_text_mode(mut session: AgentSession, message: &str, verbose: bool) -> i32 {
    let has_error = Arc::new(AtomicBool::new(false));
    let err_flag = has_error.clone();

    let listener: Arc<dyn Fn(AgentEvent, Option<tokio::sync::watch::Receiver<bool>>) -> Pin<Box<dyn std::future::Future<Output = ()> + Send>> + Send + Sync> =
    ...
    if has_error.load(Ordering::SeqCst) { 1 } else { 0 }
}

/// Run in JSON mode: newline-delimited JSON event stream.
async fn run_json_mode(mut session: AgentSession, message: &str) -> i32 {
    let has_error = Arc::new(AtomicBool::new(false));
    let err_flag = has_error.clone();

    println!("{}", serde_json::json!({"type": "start", "message": message}));
        ...
        Arc::new(move |event: AgentEvent, _signal| {
            ...
            Box::pin(async move {
                let json = match &event {
                    ...
                    AgentEvent::ToolExecutionEnd { tool_name, tool_call_id, result, is_error } => {
                        if *is_error { err_flag.store(true, Ordering::SeqCst); }
